...
public class ZeroKnowledgeProof implements PairingProvider {
    ...
    public static boolean Verify1_2_2(List<Element> proof1_2_2) {
        ...
        Element alphaSum=G.newOneElement();
        for (int i = 0; i < alphaPlusSetlength - 1; i++) {
//            right=right.mul(g.powZn(alphaPlusSet[i])).getImmutable();
            alphaSum.add(alphaPlusSet[i]);
        }
        right=right.powZn(alphaSum).getImmutable();

        return left.equals(right);
    }
    //方法注释：Hi(pp, x, a1, c1, . . . , ci−1, ai)
    public static List<Element> Proof1_3(Element g,Element h,int[] bSet) {
        //v0~vn3,challenge1:alpha,(Sa,SbSet,Sc),challenge2:beta,gammaPlus,bPlusSet,phiPlusSet
